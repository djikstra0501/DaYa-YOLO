...
class PGIDetectionLoss:
    """A Literal Copy of v8DetectionLoss for PGI experiments."""

    def __init__(self, model, tal_topk: int = 10):  # model must be de-paralleled
        """Initialize PGI Experiments with model parameters and task-aligned assignment settings."""
        device = next(model.parameters()).device  # get model device
        h = model.args  # hyperparameters
        ...
        self.bce = nn.BCEWithLogitsLoss(reduction="none")
        self.hyp = h
        self.stride = None  # model strides
        self.nc = m.nc  # number of classes
        self.no = m.nc + m.reg_max * 4
        self.reg_max = m.reg_max
        self.device = device
    ...
    def __call__(self, preds: Any, batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.stride is None:
            # infer from feature maps + input tensor
            ih = batch["img"].shape[-2]
            s = [ih / f.shape[-2] for f in feats]
            self.stride = torch.tensor(s, device=self.device, dtype=torch.float32)
...
class DualDDetectLoss:
    def __init__(self, model, aux_weight: float = 0.5, tal_topk: int = 10):
        self.main = PGIDetectionLoss(model, tal_topk=tal_topk)
        self.aux = PGIDetectionLoss(model, tal_topk=tal_topk)
